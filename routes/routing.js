        ...
        if (price) {
          ...
          if(Number(price)<=j.expectedPrice && j.expectedPrice!=j.mailPrice)
          {
            ...
            const message = {
              ...
              html: `<h2>Congratulations,</h2><h3>Price has been reduced to ₹${price}</h3><h3>Item Name- ${i.name}</h3><h3>Price(when added)- ₹${j.whenAddedPrice}</h3><h3>Expected Price- ₹${j.expectedPrice}</h3><p>Please click on the button below to check out the product</p><br><a href=${j.url} style="margin-top:10px;color:white;background-color:rgb(33, 37, 41);padding:10px 20px;border-radius:50px;text-decoration:none ">CLICK ME</a><br><br><p>If this doesn't work please visit <a href="http://localhost:3333/viewtrackings">My Trackings</a></p><br><br><h3>Thank you for joining with us</h3> `,
            };
...
routes.post("/addtracker", auth, async (req, res, next) => {
  ...
  try {
    const rest = await axios.post('http://localhost:1111/getemail',{
